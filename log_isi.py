...
def logisi_pasq_method():
    pass

def _break_calc():
    pass

def _find_thresh(histogram: list[float], isi_threshold=100):
    pass

def _add_brs():
    """
    Add burst-related spike to edges of bursts
...
def _get_peaks(histogram: list[float], pd: int = 2, threshold: float = 0, np=None) -> list[dict[str, float]]:

    """
    Finds peaks in logISI histogram

    Args:
        histogram: created by numpy.histogram
        pd: ??
        threshold: ??
        np: ??

    Returns:
        a list of dicts each containing peak and location (pks, locs)

    Raises:
    """
    # bins = list(range(0, math.ceil(spike_train[-1] / 100) * 100 + 1, 2000))
    ...
    h_density: list[float] = histogram[0]
    print(f"H_density: {h_density}")
    l: int = len(h_density)
    np: int = l if np is None else np
    ...
    while ((j < l) and (m < np)):
        ...
        endl = max(1, j-pd) - 1
        print(f"locs[m] = {locs}")
        if (m > 0 and j < min(locs[m]+pd, l-1)):
            print("HERE")
            j = min(locs[m]+pd, l-1)
            endl = j-pd
        endr = min(l, j+pd) - 1
        # initialize with zers
        temp = [0] * (endr - endl + 1)
        print(f"j: {j}, l: {l}, pd: {pd}, endl: {endl}, endr: {endr}, temp: {temp}")
        aa = range(endl, endr).index(j) - 1
        temp[aa]= -math.inf
        if (pd>1):
            idx1 = max(0, aa-2)
            idx2 = min(aa+2, len(temp))
            # https://docs.python.org/3/library/operator.html#operator.itemgetter
            range_1_2 = itemgetter(slice(0, idx1+1), slice(idx2-1, len(temp)))
            range_1_2_list = [ x + threshold for x in sum(range_1_2(temp), []) ]
            idx3 = max(0, aa-1)
            idx4 = min(aa+1, len(temp))
            range_3_4 = itemgetter(slice(idx3, idx4))
            range_3_4_list = [ x + threshold for x in range_3_4(temp) ]
            if (all( x > h_density[j] for x in range_1_2_list) and all( x > h_density[j] for x in range_1_2_list) and  j!=1  and  j!=L):
                m = m+1
                peaks[m] = h_density[j]
                locs[m] = j
            elif all( x > h_density[j] for x in range_1_2_list):
                m = m+1
                peaks[m] = h_density[j]
                locs[m] = j
