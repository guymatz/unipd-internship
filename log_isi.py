...
def _get_peaks(histogram: list[float], pd: int = 2, threshold: float = 0, np=NULL):
    ...
    while((j<l) and (m<np)):
        ...
        if (m>0  and  j<min(c(locs[m]+pd, l-1))):
            ...
            endl = j-pd
        endr = min(l, j+pd)
        aa = [ idx for idx, x in enumerate(h_density) if j == idx ]
        temp = [ -math.inf if (idx+1) in aa else 0 for idx, x in enumerate(h_density[endl:endr+1]) ]
        temp[aa] = -Inf
        if (pd>1):
            idx1 = max(1, aa-2)
            ...
            if (sum((h_density[j]>(temp[c(1:idx1, idx2:length(temp))]+Th))==FALSE)==0  and  sum((h_density[j]>(temp[idx3:idx4]))==FALSE)==0  and  j!=1  and  j!=L):
                m = m+1
                peaks[m] = h_density[j]
                locs[m] = j
        elif (sum((h_density[j]>(temp+Th))==FALSE)==0 ) :
            m = m+1
            peaks[m] = h_density[j]
            locs[m] = j
